...
class TenantIsolationTest {
    ...
    void test02_globalEnabledInjects() {
        ...
        assertTrue(result.contains(":myjdbcTenantId"),
                "全局开启时应注入 :myjdbcTenantId 占位符");
    ...
    void test03_singleTableInjection() {
        ...
        assertTrue(result.contains("user.tenant_id = :myjdbcTenantId"),
                "应在 WHERE 注入 user.tenant_id = :myjdbcTenantId");
    ...
    void test04_aliasedTableInjection() {
        ...
        assertTrue(result.contains("u.tenant_id = :myjdbcTenantId"),
    ...
    void test06_leftJoinTenantInOn() {
        ...
        assertEquals(1, tenantCount, "只有 user 表有 tenant_id，:myjdbcTenantId 应只出现一次");
    }
    ...
    void test07_bothTablesHaveTenant() {
        ...
        try {
            ...
            // role（LEFT JOIN） → ON
            assertTrue(result.contains("r.tenant_id = :myjdbcTenantId"),
    ...
    void test10_tenantAwareSpsProvidesTenantId() {
        ...
        var sps = new TenantAwareSqlParameterSource(original, "myjdbcTenantId", 42L);

        assertTrue(sps.hasValue("myjdbcTenantId"), "应能找到 myjdbcTenantId 参数");
        assertEquals(42L, sps.getValue("myjdbcTenantId"), "tenantId 值应为 42");
    }
    ...
    void test11_tenantAwareSpsDelegate() {
        ...
        var sps = new TenantAwareSqlParameterSource(original, "myjdbcTenantId", 5L);

        assertTrue(sps.hasValue("userId"), "原始参数 userId 应可访问");
        assertEquals(100L, sps.getValue("userId"), "userId 值应正确委托");
        assertEquals(5L, sps.getValue("myjdbcTenantId"), "tenantId 应为 5");
    }
    ...
    void test13_withoutTenantLambda() {
        ...
        assertTrue(normal.contains(":myjdbcTenantId"), "恢复后应正常注入");
    }
    ...
    void test16_idempotency() {
        ...
        int count = countOccurrences(result, ":myjdbcTenantId");
        assertEquals(1, count, "已有租户条件时，:myjdbcTenantId 不应重复注入，当前出现次数=" + count);
    }
    ...
    void test19_deleteAndTenantCombined() {
        ...
        assertTrue(withBoth.contains("delete_flag"),
                "应包含逻辑删除条件");
        assertTrue(withBoth.contains("tenant_id = :myjdbcTenantId"),
                "应包含租户条件");
    ...
    void test20_joinDeleteAndTenantCombined() {
        ...
        try {
            ...
            assertTrue(withBoth.contains("u.tenant_id = :myjdbcTenantId"), "user 租户条件应在 WHERE");
            // role LEFT JOIN：is_deleted 和 tenant_id 都在 ON
            assertTrue(withBoth.contains("r.is_deleted"), "role 删除条件应在 ON");
            assertTrue(withBoth.contains("r.tenant_id = :myjdbcTenantId"), "role 租户条件应在 ON");
        } finally {
            TableCacheManager.clearCache();
            TableCacheManager.initCache("io.github.canjiemo.base.myjdbc.test.entity");
            TableCacheManager.registerTenantTable("user");
    ...
    void test21_customTenantColumnName() {
        ...
        try {
            String result = appendTenantCondition("SELECT * FROM user");
            assertTrue(result.contains("org_id = :myjdbcTenantId"),
                    "自定义列名时应使用 org_id 而非 tenant_id");
            assertFalse(result.contains("tenant_id = :myjdbcTenantId"),
                    "不应再出现默认的 tenant_id");
        } finally {
    ...
    void test22_subQueryInjection() {
        ...
        String result = appendTenantCondition(sql);
        assertTrue(result.contains("tenant_id = :myjdbcTenantId"),
    ...
    void test24_appendConditionsEquivalentToStepByStep() {
        ...
        assertTrue(combined.contains("tenant_id = :myjdbcTenantId"), "合并路径应包含租户条件");
        assertTrue(stepByStep.contains("delete_flag"), "分步路径应包含逻辑删除条件");
        assertTrue(stepByStep.contains("tenant_id = :myjdbcTenantId"), "分步路径应包含租户条件");
    }
    ...
    void test25_appendConditionsWithTenantDisabledStillInjectsDelete() {
        ...
        try {
            ...
            assertFalse(result.contains(":myjdbcTenantId"), "租户关闭时不应注入租户条件");
        } finally {
            tenantEnabled = true;
        }
    }
    ...
    void test26_appendConditionsJoinBothConditions() {
        ...
        try {
            ...
            assertTrue(result.contains("u.tenant_id = :myjdbcTenantId"), "user 租户条件应在 WHERE");
            // role LEFT JOIN：删除+租户 在 ON
            assertTrue(result.contains("r.is_deleted"), "role 删除条件应在 ON");
            assertTrue(result.contains("r.tenant_id = :myjdbcTenantId"), "role 租户条件应在 ON");
            // :myjdbcTenantId 应出现 2 次（user 和 role 各一次）
            assertEquals(2, countOccurrences(result, ":myjdbcTenantId"),
                    ":myjdbcTenantId 应在 user 和 role 各出现一次");
        } finally {
            TableCacheManager.clearCache();
            TableCacheManager.initCache("io.github.canjiemo.base.myjdbc.test.entity");
            TableCacheManager.registerTenantTable("user");
    ...
    void test29_insertSqlTenantAppended() {
        ...
        assertEquals("INSERT INTO user(name, email, tenant_id) VALUES (:name, :email, :myjdbcTenantId)",
                result, "应在列列表和值列表末尾追加 tenant_id / :myjdbcTenantId");
    }
    ...
    void test32_insertSqlCustomColumn() {
        ...
        try {
            ...
            assertTrue(result.contains(":myjdbcTenantId"), "占位符应为 :myjdbcTenantId");
        } finally {
            tenantColumn = "tenant_id";
        }
    }
    ...
    void test33_insertSqlSingleField() {
        String sql = "INSERT INTO user(name) VALUES (:name)";
        String result = appendTenantToInsertSql(sql);
        assertEquals("INSERT INTO user(name, tenant_id) VALUES (:name, :myjdbcTenantId)",
