...
public class SchemaValidationRunner implements InitializingBean, Ordered {
    ...
    public SchemaValidationRunner(MyJdbcProperties properties) {
        ...
        try {
            ...
            if (result.hasErrors()) {
                ...
                if (failOnValidationError) {
                    throw new IllegalStateException("数据库模式验证失败，应用启动终止。可通过 myjdbc.fail-on-validation-error=false 或 -Dmyjdbc.fail-on-validation-error=false 禁用此行为");
                }
            } else {
                log.info("数据库模式验证通过 ✓");
            }
